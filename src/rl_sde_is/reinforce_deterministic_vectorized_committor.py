...
def sample_loss_vectorized(env, model, K, control_hjb):
    ...
    #    policy_l2_error_t = np.zeros(K)
    #else:
    #    control_hjb = np.empty_like(env.state_space_h)
...
def main():
    ...
    env = DoubleWellCommittor1D(alpha=args.alpha, beta=args.beta)
