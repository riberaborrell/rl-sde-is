...
class DoubleWellDeterministicTime1D():

    def __init__(self, beta=1., alpha=1., dt=0.005, is_state_init_sampled=False):

        # environment log name
        self.name = 'doublewell-1d-det__beta{:.1f}_alpha{:.1f}'.format(beta, alpha)

        # double well potential
        self.d = 1
        self.alpha = alpha

        # deterministic time horizont
        self.T = 1.
    ...
    def is_done(self, n):
        return np.where(n == self.N, True, False)

    def is_done_torch(self, n):
        return torch.where(n == self.N, True, False)

    def f(self, state):
        return np.zeros(state.shape[0])

    def g(self, state):
        return self.nu * (state -1) ** 2

    def f_torch(self, state):
        return torch.zeros(state.shape[0])

    def g_torch(self, state):
        return self.nu_torch * (state -1) ** 2
    ...
    def reset(self, batch_size=1):
        if not self.is_state_init_sampled:
            ...
            return np.full((batch_size, self.d), np.random.uniform(self.state_space_bounds[0], self.lb, (self.d,)))
    ...
    def reward_signal_state_action(self, state, action, n):
        done = self.is_done(n)
        reward = np.where(
            done,
            - self.g(state),
    ...
    def step(self, state, action, n):
        ...
        r, done = self.reward_signal_state_action(state, action, n)
