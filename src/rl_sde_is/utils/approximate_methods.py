...
def evaluate_time_dependent_det_policy_model(env, model, time):
    states = env.state_space_h
    time = time * np.ones(env.n_states).reshape(states.shape[:-1]+(1,))
    time_and_states = np.concatenate((time, states), axis=-1)
