...
from rl_sde_is.approximate_methods import compute_v_table_1d
...
def mc_prediction(env, policy, gamma=1.0, n_episodes=100, lr=0.01,
                  n_steps_lim=1000, test_freq_episodes=10, seed=None,
                  value_function_opt=None, load=False, live_plot=False):

    ''' Monte Carlo learning for policy evaluation with function approximation
    '''
    ...
    if live_plot:
        value_function = compute_v_table_1d(env, model)
    ...
    for ep in np.arange(n_episodes):
        ...
        for k in np.arange(n_steps_lim):
            ...
            #action = torch.FloatTensor(policy[[state_idx]])

            # step dynamics forward
            #next_state, r, done, _ = env.step_torch(state, action)
        ...
        for k in np.arange(n_steps_trajectory):
            ...
            # loss
            MSE = nn.MSELoss()
            state = torch.FloatTensor(states[k])
            state_value = model.forward(state)
            with torch.no_grad():
                target = torch.FloatTensor(ep_returns[k:k+1])
...
def main():
    ...
    # run gradient Monte Carlo learning agent following optimal policy
    #data = mc_prediction(
    data, model = mc_prediction(
        env,
